<div class="p-4 w-[500px] max-h-[80vh] overflow-y-auto bg-white shadow-lg rounded-lg border border-gray-200 text-black relative" id="cart-dropdown">
    ...
    <div id="cart-items">
        @if(session('cart') && count(session('cart')) > 0)
            ...
            @foreach(session('cart') as $key => $item)
                <div class="grid grid-cols-4 gap-4 items-center mb-4 cart-item" data-key="{{ $item['id'] }}">  {{-- Use item ID as key --}}
                    ...
                    <div class="text-center">
                        <input type="number" value="{{ $item['quantity'] }}" min="1"
                               data-key="{{ $item['id'] }}" onchange="updateQuantity(this)"  {{-- Pass the input element --}}
                               class="w-16 text-center border border-gray-300 rounded quantity-input">
                    </div>
                    ...
                    <div class="text-right">
                        <button class="remove-item text-red-500 hover:text-red-700" data-key="{{ $item['id'] }}">  {{-- Use item ID as key --}}
    ...
    <script>
        ...
        document.getElementById('cart-dropdown').addEventListener('change', function(event) { // Listen for change events
            if (event.target.classList.contains('quantity-input')) {
                const key = event.target.dataset.key;
                updateQuantity(event.target); // Pass the input element
            }
        });
        ...
        function updateQuantity(input) { // Receive the input element
