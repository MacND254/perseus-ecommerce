...
<div id="cart-dropdown" class="hidden absolute right-0 mt-2 w-96 max-h-[80vh] bg-white rounded-lg shadow-lg overflow-y-auto z-[9999] p-4">
    <div id="cart-content"> <!-- Added Wrapper for Dynamic Updates -->
        @if(count($cart) > 0)
            ...
            <div class="mt-4 flex justify-between space-x-2">
                <button id="clear-cart" class="w-1/2 bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600">
                    Clear Cart
                </button>
                <a href="{{ route('checkout') }}" class="w-1/2 text-center bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                    Checkout
                </a>
            </div>
        @else
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Function to re-bind event listeners after cart updates
            ...
            document.querySelectorAll('.remove-item').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    .then(data => {
                        ...
                        rebindCartEvents(); // Rebind after update
                    })
        ...
        rebindCartEvents(); // Initial binding
