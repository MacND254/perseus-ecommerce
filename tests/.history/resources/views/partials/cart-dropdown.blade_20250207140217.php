<div class="p-4 w-[500px] max-h-[80vh] overflow-y-auto bg-white shadow-lg rounded-lg border border-gray-200 text-black relative" id="cart-dropdown">
    ...
    <div id="cart-items">
        @if(session('cart') && count(session('cart')) > 0)
            ...
            @foreach(session('cart') as $key => $item)  {{-- Iterate through the session cart --}}
                <div class="grid grid-cols-4 gap-4 items-center mb-4 cart-item" data-key="{{ $item['id'] }}">
                    ...
                    <div class="text-center">
                        <input type="number" value="{{ $item['quantity'] }}" min="1"
                               data-key="{{ $item['id'] }}" onchange="updateQuantity(this)"
                    ...
                    <div class="text-right">
                        <button class="remove-item text-red-500 hover:text-red-700" data-key="{{ $item['id'] }}">
    ...
    <script>
        ...
        document.getElementById('cart-dropdown').addEventListener('change', function(event) {
        ...
        function removeCartItem(key) {
            fetch(`/cart/remove`, { // Correct URL (no key in URL)
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ id: key }) // Key is sent in the body
            })
        ...
        function updateQuantity(input) {
            ...
            fetch(`/cart/update-quantity/${key}`, { // Correct URL (key in URL)
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Content-Type': 'application/json'
