<div class="p-4 w-[500px] max-h-[80vh] overflow-y-auto bg-white shadow-lg rounded-lg border border-gray-200 text-black">
        ...
        @if(session('cart') && count(session('cart')) > 0)
            ...
            @foreach(session('cart') as $key => $item)
                <div class="grid grid-cols-4 gap-4 items-center mb-4" data-key="{{ $key }}">
                    ...
                    <div class="text-right">
                        <button onclick="removeCartItem('{{ $key }}')" class="text-red-500 hover:text-red-700">
    ...
    <div class="border-t pt-4">
        ...
        <button onclick="clearCart()" class="w-full mt-4 bg-red-500 text-white py-2 rounded hover:bg-red-600">Clear Cart</button>
    </div>
    ...
    <script>
        function removeCartItem(key) {
            fetch(`/cart/remove/${key}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, // Crucial!
