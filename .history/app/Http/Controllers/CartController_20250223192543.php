...
public function showCartDropdown()
{
    ...
    foreach ($cartItems as $item) {
        // Decode the JSON (ensure it is an array)
        $images = is_array($item->product->product_image) ? $item->product->product_image : json_decode($item->product->product_image, true);

        // Set the first image or default placeholder
        $item->first_image = !empty($images) ? asset('storage/' . $images[0]) : asset('images/default-placeholder.png');
    }
