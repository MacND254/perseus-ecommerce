...
public function showCartDropdown()
{
    ...
    foreach ($cartItems as $item) {
        ...
        \Log::info('Product Images:', ['images' => $images]); // Log for debugging

        // Check if images array exists and has data
        if (!empty($images) && is_array($images)) {
            $item->firstImage = asset($images[0]); // Use the first image
        } else {
            $item->firstImage = asset('default-image.jpg'); // Fallback image
        }
    }
