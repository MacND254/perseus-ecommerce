...
public function showCartDropdown()
{
    ...
    foreach ($cartItems as $item) {
        // Decode the JSON image column
        $images = json_decode($item->product->product_image, true);

        // Check if it's a valid array and get the first image
        if (is_array($images) && !empty($images)) {
            $item->first_image = asset('storage/' . $images[0]);
        } else {
            $item->first_image = asset('images/default-placeholder.png'); // Default if no image
        }
    }
