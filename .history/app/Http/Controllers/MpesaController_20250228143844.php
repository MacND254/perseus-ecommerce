...
class MpesaController extends Controller {
    ...
    {
        Log::info('M-Pesa Confirmation Request:', $request->all()); // Log all incoming data

        // Ensure required values are not null
        $mpesa_receipt_number = $request->input('TransID', 'N/A'); // Default to 'N/A' if null
        $amount = $request->input('TransAmount', 0); // Default to 0 if null
        $phone_number = $request->input('MSISDN', 'N/A');

        // Validate required fields before saving
        if ($amount == 0) {
            Log::error('M-Pesa Confirmation Error: Amount is missing or null', $request->all());
            return response()->json([
                "ResultCode" => 1,
                "ResultDesc" => "Failed - Amount missing"
            ]);
        }
