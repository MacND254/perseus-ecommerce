...
// User Authentication (Jetstream Default)
// User Login Route (Redirects to Dashboard if Authenticated)
Route::middleware(['guest'])->group(function () {
    Route::get('/login', function () {
        return redirect()->route('dashboard')->with('message', 'You are already logged in.');
    })->middleware('auth')->name('login');

    Route::view('/login', 'auth.login')->name('login'); // Jetstream's default login form
});

// Jetstream Authenticated User Routes (Dashboard)
Route::middleware([
    'auth:sanctum',
    ...
    Route::get('/dashboard', function () {
        return view('dashboard'); // Jetstream's default user dashboard
    })->name('dashboard');
});
Route::middleware(['auth'])->group(function () {
    Route::get('/profile', function () {
        return view('profile.show'); // Ensure this view exists
    })->name('profile');
});
    ...
    Route::middleware(['auth'])->group(function () {
        ...
        // Fetch cart data
Route::get('/cart/data', [CartController::class, 'fetchCartData'])->middleware('auth');

// Update cart item quantity
Route::post('/cart/update/{cartItemId}', [CartController::class, 'updateQuantity'])->middleware('auth');

// Remove item from cart
Route::delete('/cart/remove/{cartItemId}', [CartController::class, 'removeItem'])->middleware('auth');

// Clear cart
Route::delete('/cart/clear', [CartController::class, 'clearCart'])->middleware('auth');
...
Route::prefix('admin')->group(function () {
    ...
    Route::post('/register', [RegisterController::class, 'register']); // Fix route name

    // Admin Protected Routes
    Route::middleware(['admin.auth'])->group(function () {
        Route::get('/dashboard', function () {
            return view('admin.dashboard'); // ✅ Loads admin dashboard correctly
...
//Route::prefix('admin')->name('admin.')->group(function () {
   ...
   // })->name('dashboard');
    ...
    //Route::get('/categories_tags', [PostCategoryController::class, 'index'])->name('categories_tags');
...
Route::get('/careers', [CareerController::class, 'index'])->name('career.index');
Route::get('/careers/{id}', [CareerController::class, 'show'])->name('careers.show');
//Route::post('/careers/{id}/apply', [CareerController::class, 'apply'])->name('career.apply.store');
