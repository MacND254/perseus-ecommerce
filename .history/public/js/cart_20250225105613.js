...
document.addEventListener("click", function (event) {
    if (event.target.id === "clearCartButton") { // Ensure it targets the correct button
        fetch(`${baseUrl}/cart/clear`, { // Use dynamically generated base URL
            method: "POST",
            headers: {
                "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute("content"),
                "Accept": "application/json"
        ...
        .then(data => {
            if (data.message) {
                document.querySelectorAll(".cart-item").forEach(item => item.remove()); // Remove all cart items from DOM
                fetchCartItems(); // Refresh cart totals
            } else {
