    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="col-span-1">
            ...
            <form id="addToCartForm">
                ...
                <input type="number" id="quantity" name="quantity" value="1" min="1" class="mt-1 w-20 p-2 border rounded text-black">

                @auth
                    <!-- Show button if user is logged in -->
                    <button type="submit" class="bg-blue-500 text-black px-6 py-2 rounded hover:bg-blue-600 mt-4">
                        Add to Cart
                    </button>
                @else
                    <!-- Redirect to login if user is not authenticated -->
                    <a href="{{ route('login') }}" class="bg-blue-500 text-black px-6 py-2 rounded hover:bg-blue-600 mt-4 inline-block">
                        Add to Cart
                    </a>
                @endauth
            </form>
...
<div class="p-4 bg-gray-100 rounded-lg shadow-md mb-6">
    ...
    @php
        $userContact = Auth::check() ? \App\Models\UserContact::where('user_id', Auth::id())->first() : null;
    @endphp
    <p>{{ $userContact ? $userContact->address : 'No address provided' }}</p>
</div>
            ...
            <!-- Customer Reviews -->
            <h2 class="text-lg font-bold mt-6 text-white">Customer Reviews</h2>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mt-4">
        @foreach($relatedProducts as $related)
            <a href="{{ route('products.show', $related->id) }}" class="bg-white p-4 rounded shadow-md transition hover:shadow-lg hover:scale-105 text-black">
                <img src="{{ asset('storage/' . json_decode($related->product_image, true)[0] ?? 'default.jpg') }}"
                     alt="{{ $related->name }}" class="w-full h-40 object-contain rounded text-black">
                <p class="text-center mt-2 font-semibold text-black">{{ $related->name }}</p>
            </a>
        @endforeach
    </div>
</section>
