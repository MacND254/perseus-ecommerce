<div id="cartDropdown" class="absolute right-0 mt-2 w-80 bg-white border rounded-lg shadow-lg p-4 hidden z-50">
    <div id="cartItemsContainer" class="text-black">
        <h4 class="text-xs font-bold text-black">Cart</h4>
        @if(count($cartItems) > 0)
            @foreach($cartItems as $item)
                <div class="cart-item flex items-center space-x-4 p-2 border-b">

                    <img src="{{ $item->product->first_image }}" alt="{{ $item->product->name }}" class="w-16 h-16 object-cover rounded">
                    <div class="flex-1">
