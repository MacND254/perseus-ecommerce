        ...
        @if(count($cartItems) > 0)
            @foreach($cartItems as $item)
                @php
                    ...
                    // Get the first image if available, else use a placeholder
                    $firstImage = !empty($images) ? asset($images[0]) : asset('images/default-placeholder.png');
                @endphp

                <div class="cart-item flex items-center space-x-4 p-2 border-b">
                    <img src="{{ $firstImage }}" alt="{{ $item->product->name }}" class="w-16 h-16 object-cover rounded">
                    <div class="flex-1">
