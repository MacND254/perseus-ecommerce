...
<nav class="bg-gray-800 text-white shadow-md">
    <div class="max-w-7xl mx-auto flex justify-between items-center py-3 px-4">
        ...
        <ul>
            <li class="relative">
                <button id="cartButton" class="relative p-2">
                    🛒
                    <span id="cartCount" class="absolute top-0 right-0 bg-red-500 text-white text-xs px-2 rounded-full">{{ $cartCount }}</span>
        ...
        <!-- Include JavaScript to handle cart interactions -->
        <script>
            // Toggle cart dropdown visibility
           // Cart dropdown toggle functionality
document.getElementById('cartButton').addEventListener('click', function() {
    var cartDropdown = document.getElementById('cartDropdown');
    cartDropdown.classList.toggle('hidden'); // Toggle visibility of the cart dropdown

    // Optionally, update the cart count dynamically (in case items are added/removed via AJAX)
    var cartCount = document.getElementById('cartCount');
    cartCount.textContent = parseInt(cartCount.textContent) + 1; // Update the cart count as needed
});
// After adding an item to the cart via AJAX, update the cart count:
document.getElementById('cartCount').textContent = updatedCartCount; // updatedCartCount is the new number
            ...
            // Update the cart dropdown with fetched data
            function updateCart(data) {
                let cartCount = document.getElementById('cartCount');
                cartCount.textContent = data.cartItems.length;  // Update item count
                    ...
                    let cartItemHTML = `
                        <div class="flex justify-between items-center border-b py-2">
                            <div class="flex items-center space-x-2">
                                <img src="${item.productImage}" alt="${item.productName}" class="w-12 h-12 object-cover rounded-md">
                                <span class="font-semibold">${item.productName}</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                <input type="number" class="w-12 p-1 border rounded" value="${item.quantity}" min="1" data-cart-item-id="${item.id}" id="quantity-${item.id}">
                                <span class="text-sm">Ksh. ${item.subtotal}</span>
                ...
                cartVat.textContent = data.vat;
            }

            // Adjust item quantity
            document.addEventListener('change', function (event) {
                if (event.target && event.target.id.startsWith('quantity-')) {
                    let cartItemId = event.target.getAttribute('data-cart-item-id');
