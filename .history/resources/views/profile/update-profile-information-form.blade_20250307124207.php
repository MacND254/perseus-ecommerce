<x-form-section submit="updateProfileInformation">
    <x-slot name="title">
        <span class="text-red-500">{{ __('Profile Information') }}</span>
    </x-slot>

    <x-slot name="description">
        <span class="text-white">{{ __('Update your account\'s profile information, phone number, address, and email address.') }}</span>
    </x-slot>
    ...
    <x-slot name="form">
        ...
        <div class="col-span-6 sm:col-span-4 text-black">
            ...
            <x-input id="email" type="email" class="mt-1 block w-full" wire:model="state.email"
                     value="{{ auth()->user()->email }}" autocomplete="username" />
            ...
            @if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::emailVerification()) && ! auth()->user()->hasVerifiedEmail())
                ...
                @if (session()->has('verificationLinkSent'))
