<x-form-section submit="updateUserContactInformation">
    ...
    <x-slot name="description">
        ...
        <div class="p-4 bg-gray-800 rounded-lg text-white">
            ...
            <p><strong>{{ __('Phone:') }}</strong> <span wire:loading.remove>{{ $state['phone'] }}</span></p>
            <p><strong>{{ __('Address:') }}</strong> <span wire:loading.remove>{{ $state['address'] }}</span></p>
            <p><strong>{{ __('City:') }}</strong> <span wire:loading.remove>{{ $state['city'] }}</span></p>
            <p><strong>{{ __('State:') }}</strong> <span wire:loading.remove>{{ $state['state'] }}</span></p>
            <p><strong>{{ __('Country:') }}</strong> <span wire:loading.remove>{{ $state['country'] }}</span></p>
            <p><strong>{{ __('Postal Code:') }}</strong> <span wire:loading.remove>{{ $state['postal_code'] }}</span></p>

            <span wire:loading class="text-gray-400">Loading...</span>
        </div>
    </x-slot>
