...
<nav class="bg-gray-800 text-white shadow-md">
    ...
    <!-- Hamburger Menu for Mobile -->
<div id="menu-toggle" class="md:hidden cursor-pointer flex flex-col space-y-1.5 p-2">
...
<div id="mobile-menu" class="fixed inset-0 bg-gray-800 bg-opacity-95 z-50 transform transition-transform duration-300 ease-in-out translate-x-full">
    <div class="relative bg-gray-700 h-full w-3/4 sm:w-1/2 px-4 py-2">
        <!-- Close Button -->
        <button id="menu-close" class="absolute top-4 right-4 text-white text-2xl font-bold hover:text-gray-300 transition-colors duration-200">
            &times;
        </button>

        <!-- Mobile Menu Links -->
        <div class="mt-12">
            <a href="{{ route('home') }}" class="py-3 block border-b border-gray-600 hover:bg-gray-600 text-white transition-colors duration-200">Home</a>
            <a href="{{ route('about-us.index') }}" class="py-3 block border-b border-gray-600 hover:bg-gray-600 text-white transition-colors duration-200">About Us</a>
            <a href="{{ route('services.index') }}" class="py-3 block border-b border-gray-600 hover:bg-gray-600 text-white transition-colors duration-200">Our Services</a>
            <a href="{{ route('career.index') }}" class="py-3 block border-b border-gray-600 hover:bg-gray-600 text-white transition-colors duration-200">Career</a>
            <a href="{{ route('blog.index') }}" class="py-3 block border-b border-gray-600 hover:bg-gray-600 text-white transition-colors duration-200">Blog</a>
            <a href="{{ route('products.index') }}" class="py-3 block border-b border-gray-600 hover:bg-gray-600 text-white transition-colors duration-200">Shop</a>
            <a href="{{ route('contact-us.index') }}" class="py-3 block border-b border-gray-600 hover:bg-gray-600 text-white transition-colors duration-200">Contact Us</a>
        </div>

        <!-- Auth Links for Mobile -->
        <div class="flex flex-col space-y-2 mt-4">
            @guest
                <a href="{{ route('login') }}" class="px-4 py-2 rounded-lg font-semibold text-sm bg-purple-600 text-white hover:bg-purple-700 transition-colors duration-200">Sign In</a>
                <a href="{{ route('register') }}" class="px-4 py-2 rounded-lg font-semibold text-sm bg-purple-600 text-white hover:bg-purple-700 transition-colors duration-200">Sign Up</a>
            @else
                <a href="{{ route('dashboard') }}" class="px-4 py-2 rounded-lg font-semibold text-sm bg-gray-600 text-white hover:bg-gray-700 transition-colors duration-200">Dashboard</a>
                <a href="{{ route('profile.show') }}" class="px-4 py-2 rounded-lg font-semibold text-sm bg-gray-600 text-white hover:bg-gray-700 transition-colors duration-200">Profile</a>
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full px-4 py-2 rounded-lg font-semibold text-sm bg-gray-600 text-white hover:bg-gray-700 transition-colors duration-200 text-left">Log Out</button>
                </form>
            @endguest
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (menuToggle && mobileMenu && menuClose) {
            ...
            document.addEventListener('click', (event) => {
                if (
                    !mobileMenu.contains(event.target) && // Click is outside menu
                    !menuToggle.contains(event.target) // Click is not on the hamburger button
                ) {
                    mobileMenu.classList.add('translate-x-full'); // Hide menu
                }
            });
